...
class Calculator(Node): # in: 71, 72, 78, 117, out: 110, 128

    def __init__(self): # 외부에서 0101의 데이터가 들어오면, 받는 역할
        ...
        self.operator = ['+', '-', '*', '/']
        self.callback_group = ReentrantCallbackGroup() # main에서 multi threading 가능하게
        ...
        self.arithmetic_service_server = self.create_service(
            ArithmeticOperator,                     # message type
            'arithmetic_operator',                 # service name               
            self.get_arithmetic_operator,           # service callback
            callback_group=self.callback_group)     # callback group
    ...
    def get_arithmetic_operator(self, request, response): # service callback
        self.argument_operator = request.arithmetic_operator # request에서 operator 받음. 
        self.argument_result = self.calculate_given_formula( 
            self.argument_a,
            self.argument_b,
            self.argument_operator)
        response.arithmetic_result = self.argument_result    # response에 결과값.
